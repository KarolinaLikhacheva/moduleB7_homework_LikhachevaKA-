from random import randint  # импортируем генерацию случайных чисел
...
class Field:
    def __init__(self, hid=False, size=6):
        ...
        self.busy = []  # Буфер точек занятых кораблями
        self.ships = []

    # Размещаем кораблей (учитывая, что точка не занята, а если это так - срабатывает исключение)
...
class Game:
    player_1 = input('Давайте знакомиться! Меня зовут Джек Воробей. Введите Ваше имя? \n')
    ...
    #  Приветствие
    def greet(self):
        print(f'Приятно познакомиться, {Game.player_1}! Играем в игру "МОРСКОЙ БОЙ".')
        print("Выигрывает тот, кто разбомбит все корабли противника первым.")
        print(
    ...
    def loop(self):
        ...
        while True:
            ...
            print(self.us.board)
            print("Игровое поле Джека Воробья:")
            print(self.ai.board)
            if num % 2 == 0:
                repeat = self.us.move()
            else:
            ...
            if self.ai.board.defeat():  # Количиство пораженых кораблей равно количеству кораблей на доске
                print(f"{Game.player_1}, Мои поздравления, Вы выиграли!!!")
                break

            if self.us.board.defeat():
                print(f"ХА - мне жаль, но Вы {Game.player_1}, проиграли!")
                break
            num += 1
...
class RepeatInput(FeaturesGame):
    def __str__(self):
        return "Уже стреляли по этим координатам! Давай другие цифры!"
